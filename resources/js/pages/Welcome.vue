...
<template>
    ...
    <PublicLayout>
        <!-- 1. HERO CAROUSEL DINÁMICO, INTERCALADO Y ESPACIOSO -->
        <section class="max-w-7xl mx-auto px-4 py-8 mt-6">
            
            <div v-if="banners.length > 0" class="relative w-full h-[500px] md:h-[550px] rounded-[4rem] overflow-hidden bg-[#072D44] shadow-2xl">
                ...
                <TransitionGroup name="hero-fade">
                    ...
                    >
                        ...
                        <img 
                            ...
                            class="absolute top-0 w-full h-full object-cover z-0 transition-all duration-1000"
                            :class="index % 2 === 0 ? 'right-0 md:w-3/5 object-right' : 'left-0 md:w-3/5 object-left'"
                            :alt="banner.titulo"
                        />
                        ...
                        <div 
                            ...
                            :class="index % 2 === 0 
                                ? 'bg-gradient-to-r from-[#072D44] via-[#072D44]/90 to-transparent' 
                                : 'bg-gradient-to-l from-[#072D44] via-[#072D44]/90 to-transparent'"
                        ></div>


                        <div 
                            class="relative z-20 max-w-xl text-white flex flex-col"
                            :class="index % 2 === 0 ? 'text-left items-start' : 'text-right items-end'"
                        >
                            ...
                            <h1 class="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.1] uppercase tracking-tighter mb-6 italic">
                                {{ banner.titulo }}
                            </h1>


                            <p class="text-white/70 text-base md:text-lg mb-10 font-medium leading-relaxed line-clamp-2">
                                {{ banner.descripcion }}
                            </p>
                            
                            <Link 
                                :href="CatalogController.index.url()" 
                                class="bg-white text-[#072D44] w-fit px-12 py-5 rounded-2xl font-black uppercase text-xs tracking-widest transition-all hover:bg-[#9CCDDB] shadow-2xl active:scale-95"
                            >
                                Explorar Catálogo
                            </Link>
                        </div>
                ...
                <div v-if="banners.length > 1" class="absolute bottom-10 left-1/2 -translate-x-1/2 z-30 flex gap-4 items-center">
                    <button 
                        ...
                        :class="['transition-all duration-500 rounded-full', currentSlide === i ? 'w-10 h-2.5 bg-amber-500 shadow-lg' : 'w-2.5 h-2.5 bg-white/20 hover:bg-white/40']"
                    ></button>
                </div>
            </div>


            <div v-else class="w-full h-[500px] bg-navy-900 rounded-[4rem] border border-white/5 flex items-center justify-center">
        ...
        <section class="max-w-7xl mx-auto px-4 py-20 mt-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                <Link v-for="cat in categories" :key="cat.id"
                    ...
                    <span class="text-xs font-black uppercase tracking-widest text-gray-500 group-hover:text-[#072D44] text-center leading-tight">
                        {{ cat.nombre }}
                    </span>
                </Link>
            </div>
        ...
        <section class="max-w-7xl mx-auto px-4 py-10 pb-32">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
                <div v-for="prod in featuredProducts" :key="prod.sku" class="bg-white p-8 rounded-[3.5rem] border border-gray-100 hover:shadow-2xl transition-all group flex flex-col shadow-sm">
                    <div class="aspect-square bg-[#F8FAFC] rounded-[2.5rem] mb-8 overflow-hidden p-10 flex items-center justify-center relative border border-gray-100">
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        <div>
                            ...
                            <h3 class="font-black text-[#072D44] uppercase text-lg leading-tight mt-2 italic line-clamp-2 h-12">{{ prod.nombre_comercial }}</h3>
                        </div>
                        <div class="pt-8 mt-8 border-t border-gray-50 flex items-center justify-between">
                            ...
                            <button class="bg-[#072D44] hover:bg-emerald-500 text-white p-4 rounded-2xl shadow-xl transition-all active:scale-90"><LucideIcons.Plus class="size-6" /></button>
                        </div>
                    </div>
                </div>
            </div>
...
.hero-fade-enter-from { opacity: 0; transform: scale(1.02); }
.hero-fade-leave-to { opacity: 0; transform: scale(0.98); }
</style>